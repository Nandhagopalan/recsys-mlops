...
class myMerlinFlow(FlowSpec):
    ...
    def get_dataset(self):
        ...
        sampling_expression = (
            "" if snowflake_sampling == 0 else "sample({})".format(snowflake_sampling)
        )
    ...
    def join_runs(self, inputs):
        ...
        #     inp.hyper_string: inp.comet_experiment_key for inp in inputs
        # }
    ...
    def prepare_predictions_for_comet_panel(
        self, h_m_shoppers, best_predictions, item_id_2_meta, api_key, experiment_key
    ):
        ...
        for shopper in h_m_shoppers[:n_shoppers]:
            ...
            for p in cnt_predictions["items"]:
                product_type = (
                    item_id_2_meta[p]["product_group_name"]
                    if p in item_id_2_meta
                    else "NO_GROUP"
                )
    ...
    @enable_decorator(
        batch(image="public.ecr.aws/outerbounds/merlin-reasonable-scale:22.11-latest"),
        flag=os.getenv("EN_BATCH"),
    )
        ...
        if not os.environ.get("EXPORT_TO_APP", None) == "1":
            ...
            for shopper, preds in self.best_predictions.items():
                ...
                new_row = {
                    ...
                    "product_type": self.item_id_2_meta[target_item][
                        "product_group_name"
                    ],
                }
                rows.append(new_row)
    ...
    def cache_predictions(self):
        ...
        if not bool(int(os.getenv("SAVE_TO_CACHE"))):
            ...
            data = [
                {"userId": user, "recs": json.dumps(recs)}
                for user, recs in self.best_predictions.items()
            ]
            ...
            data.append(
                {
                    "userId": "no_user",
                    "recs": json.dumps(
                        ["test_rec_{}".format(_) for _ in range(int(self.TOP_K))]
                    ),
                }
            )
